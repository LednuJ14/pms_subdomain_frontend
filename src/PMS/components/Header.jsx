...
const Header = ({ userType = 'manager' }) => {
  ...
  const isImpersonating = typeof sessionStorage !== 'undefined' && 
                          sessionStorage.getItem('impersonated_subdomain') && 
                          typeof window !== 'undefined' && 
                          window.location.hostname.startsWith('admin.');

  const handleExitImpersonation = () => {
    sessionStorage.removeItem('impersonated_subdomain');
  ...
  return (
    <>
    {isImpersonating && (
      <div className="bg-amber-500 text-white px-4 py-2 flex justify-between items-center text-sm font-medium shadow-md relative z-50">
        <div className="flex items-center space-x-2">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <span>You are viewing {propertyName} as a Super Admin. Actions taken here will be applied to this property.</span>
        </div>
        <button 
          onClick={handleExitImpersonation}
          className="bg-amber-700 hover:bg-amber-800 text-white px-4 py-1 rounded transition-colors"
        >
